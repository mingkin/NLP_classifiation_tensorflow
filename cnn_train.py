...
'''
如果前后两次训练网络不同时，每次训练前要将checkpoint点删除
...
tf.app.flags.DEFINE_boolean("is_training", True, "is traning.true:tranining,false bn layer")
...
tf.app.flags.DEFINE_integer("num_filters", 16, "number of filters") #16-32-64-128-256--->512
tf.app.flags.DEFINE_boolean("is_trace_train", True, "true:trace,false:no trace")
filter_sizes = [6, 7, 8]
'path'
tf.flags.DEFINE_string("w2v", "./word_vector/word2vec.words", "word2vector_path.")
tf.flags.DEFINE_string("path", "./data/test.txt,./data/test.txt,", "Data source for  data.")
tf.app.flags.DEFINE_string("ckpt_dir" , "text_cnn_output/", "checkpoint location for the model")
...
def preprocess():
    ...
    print('Test set ....')
    if FLAGS.class_weight == 0:
        x_test, ytest = utils.read_file_seg(path_data[2])
    else:
        x_test, ytest = utils.read_file_seg_sparse(path_data[2])
...
def train(x_train, y_train, embedding_weights, x_dev, y_dev):
    ...
    with sess.as_default():
        textCNN = TextCNN(filter_sizes=filter_sizes, num_filters=FLAGS.num_filters,
                          num_classes=FLAGS.num_classes, learning_rate=FLAGS.learning_rate,
                          ...
                          embed_size=FLAGS.embed_size, is_training=FLAGS.is_training, class_weight=FLAGS.class_weight)
        ...
        #embeding layer
        assign_pretrained_word_embedding(sess, embedding_weights, textCNN)
        ...
        for batch in batches:
            ...
            if counter == 0:
                print("trainX:", len(y_batch))
            if FLAGS.is_trace_train:
                curr_loss, curr_acc, pred, _, summaries, step = sess.run(
                    [textCNN.loss, textCNN.accuracy, textCNN.predictions, textCNN.train, textCNN.train_summary_op,
                     textCNN.global_step],
                    feed_dict={textCNN.input_x: x_batch, textCNN.input_y: y_batch, textCNN.dropout_keep_prob: 0.1})
                train_summary_writer.add_summary(summaries, step)
                print('global_step:', step)
            else:
                curr_loss, curr_acc, pred, _, step = sess.run(
                    [textCNN.loss, textCNN.accuracy, textCNN.predictions, textCNN.train, textCNN.global_step],
                    feed_dict={textCNN.input_x: x_batch, textCNN.input_y: y_batch, textCNN.dropout_keep_prob: 0.1})
                print('global_step:', step)
            ...
            if counter % num_batches_per_epoch == 0:
                ...
                evaluate(sess, textCNN, x_dev, y_dev)
    ...
    for batch in batches:
        ...
        if counter % num_batches_per_epoch == 0:
            log.info("\t-\tBatch_size %d\t-\tTest Loss:%.3f\t-\tTest Accuracy:%.3f\t-\t"
                     "Test Precision:%.3f\t-\tTest Recall:%.3f " % (counter,
                                                                    loss / float(counter), acc / float(counter),
                                                                    pre / float(counter), recall / float(counter)))
            # print(time_, "\t-\tBatch_size %d\t-\tTest Loss:%.3f\t-\tTest Accuracy:%.3f\t-\t"
            #              "Test Precision:%.3f\t-\tTest Recall:%.3f " % (counter,
            #                 loss / float(counter), acc / float(counter), pre / float(counter), recall / float(counter)))
